    ...
    >
      ...
      >
        ...
        <style>{`
          ...
          @keyframes floatParticle {
            ...
            25% { transform: translateY(-18px) translateX(8px) scale(1.3); opacity: 0.35; }
            50% { transform: translateY(-8px) translateX(-8px) scale(0.85); opacity: 0.25; }
            75% { transform: translateY(12px) translateX(-12px) scale(1.15); opacity: 0.3; }
          }
          @keyframes pulse {
            0%, 100% { opacity: 0.5; }
            50% { opacity: 0.85; }
        ...
        {Array.from({ length: 25 }).map((_, i) => (
          <div
            ...
            style={{
              ...
              animation: `floatParticle ${7 + Math.random() * 14}s ease-in-out infinite`,
        ...
        <div
          style={{
            ...
            height: 320,
            borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(0,245,255,0.08) 0%, rgba(0,245,255,0) 70%)',
            zIndex: 0,
          }}
        />
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          ...
          <div
            style={{
              ...
              boxShadow: '0 0 40px rgba(0, 212, 255, 0.12), 0 8px 32px rgba(0, 0, 0, 0.25)',
            }}
          >
            <SafetyOutlined style={{ fontSize: 54, color: '#00d4ff' }} />
          </div>
          ...
          <div
            style={{
              ...
              height: 2,
              background:
                'linear-gradient(90deg, rgba(0,212,255,0), rgba(0,212,255,0.5), rgba(0,212,255,0))',
              borderRadius: 2,
              margin: '0 auto 30px',
            }}
          />
            ...
            >
              <div
                style={{
                  ...
                  height: 1,
                  background:
                    'linear-gradient(90deg, rgba(0,212,255,0), rgba(0,212,255,0.2), rgba(0,212,255,0))',
                  marginBottom: 12,
                }}
              />
              <div
      ...
      <div
        style={{
          ...
          justifyContent: 'center',
          padding: 60,
          background: 'linear-gradient(180deg, #0c1932 0%, #091222 100%)',
          position: 'relative',
        }}
      >
        {/* 背景光晕 */}
        ...
        <div style={{ width: '100%', maxWidth: 360, position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <Text style={{ color: '#6a8aaf', fontSize: 14 }}>
              请输入您的账号密码进入系统
            </Text>
          </div>
          ...
          <div
            style={{
              ...
              padding: '36px 28px',
              boxShadow: '0 0 30px rgba(0, 212, 255, 0.06), 0 4px 24px rgba(0, 0, 0, 0.3)',
            }}
          >
            <Form name="login" onFinish={onFinish} size="large" autoComplete="off">
              <Form.Item name="username" rules={[{ required: true, message: '请输入账号' }]}>
                <Input
                  prefix={<UserOutlined style={{ color: '#5a8aaf' }} />}
                  placeholder="请输入账号"
                  style={{
                    height: 50,
                    borderRadius: 10,
              ...
              <Form.Item name="password" rules={[{ required: true, message: '请输入密码' }]}>
                <Input.Password
                  prefix={<LockOutlined style={{ color: '#5a8aaf' }} />}
                  placeholder="请输入密码"
                  style={{
                    height: 50, borderRadius: 10, fontSize: 15,
                    background: 'rgba(0, 16, 40, 0.5)', border: '1px solid rgba(0, 212, 255, 0.15)', color: '#e0f0ff',
                  }}
                />
              </Form.Item>

              <div style={{ textAlign: 'right', marginTop: -12, marginBottom: 16 }}>
                <a onClick={() => setForgotOpen(true)} style={{ color: '#5a8aaf', fontSize: 13 }}>
                  忘记密码？
                </a>
              </div>
              ...
              <Form.Item style={{ marginBottom: 20 }}>
                <Button type="primary" htmlType="submit" loading={loading} block
                  style={{ height: 50, borderRadius: 10, fontSize: 16, fontWeight: 600,
                    background: 'linear-gradient(135deg, #00d4ff, #0088cc)', border: 'none',
                    boxShadow: '0 4px 24px rgba(0, 212, 255, 0.35)', letterSpacing: 8, color: '#fff' }}>
                  登录
                </Button>
              </Form.Item>
            </Form>
    ...
    <Modal title="重置密码" open={forgotOpen} onCancel={() => { setForgotOpen(false); setForgotStep('form'); }}
      ...
      {forgotStep === 'form' ? (
        <Form layout="vertical" onFinish={async (vals: any) => {
          ...
          try {
            await fetch('/api/v1/auth/send-sms-code', {
              ...
              body: JSON.stringify({ phone: vals.phone, purpose: 'reset_password' }),
            });
        ...
        <Form layout="vertical" onFinish={async (vals: any) => {
          ...
          <Form.Item name="code" label="短信验证码" rules={[{ required: true }]}>
